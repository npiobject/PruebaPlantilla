# PruebaPlantilla — instrucciones del proyecto
...
El repositorio `npiobject/PruebaPlantilla`, rama `main`, es la **única** fuente de verdad, tanto para el código como para la documentación de `docs/planificacion/`. Todo lo que importe vive aquí y se edita aquí.

Google Drive (carpeta normal `Mi unidad/PruebaPlantilla`, id `1EUL0hBVLSHsUWz9auVSXF7J-Z90hNqXl`) es **solo un destino de copias**, nunca un origen:
...
| Qué | URL | Despliegue |
|---|---|---|
| Mock estático (Pages) | https://npiobject.github.io/PruebaPlantilla/ | `.github/workflows/pages.yml` en push a `main` |
| Backend (Fly.io) | https://pruebaplantilla-npi.fly.dev/ · `/salud` | `.github/workflows/deploy.yml` en push a `main` que toque `app/**` |

## Código

- Todo cambio termina en commit + push a `main`. Mensajes de commit en español, imperativo.
- Backend en `app/` (Rust, axum + tokio), paquete `pruebaplantilla-backend`. `GET /` devuelve texto plano; `GET /salud` devuelve `{"ok":true,"build":"<BUILD_ID>"}`, donde `BUILD_ID` es el SHA que inyecta el workflow.
- Mocks estáticos en `docs/`. `docs/index.html` es el mock vivo; los anteriores se archivan en `docs/mocks/NNN-nombre.html`.
- Cada mock lleva `<meta name="build" content="PP-B1-AAAAMMDD-NNN">` con un número nuevo en cada iteración.
...
Si necesitas comprobar algo desde la sesión, hazlo contra la API de GitHub (`https://api.github.com/repos/npiobject/PruebaPlantilla/actions/runs/...`), que sí es accesible.

## Despliegue

- Estático: GitHub Pages vía `.github/workflows/pages.yml` (push a `main` publica `docs/`).
- Backend: Fly.io vía `.github/workflows/deploy.yml`, app `pruebaplantilla-npi` (región `cdg`), con el token en el secreto `FLY_API_TOKEN` del repositorio. Nunca lo imprimas en los logs.
